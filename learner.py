...
class Learner(object):
    ...
    def act(self, step):
        ...
        for team, policy in zip(self.teams_list, self.policies_list):
            ...
            # 统计需要更新的智能体数据
            num_updates = goal_done_mask.sum().item()
            ...
            if goal_done_mask.any():
                ...
                # ⭐ 优化：直接在这里生成随机 vec_inp_agents 和 map_inps，作为占位符
                vec_inp_agents = torch.randn((len(proc_indices), 5), device=self.device)  # [N, 5]
                map_inps = torch.randn((len(proc_indices), 4, 20, 20), device=self.device)  # [N, 4, H, W]
            ...
            # 生成一个占位符all_vec_inps
            all_vec_inps = torch.randn((num_processes, num_agents, 5), device=self.device)  # [num_processes, num_agents, 5]
        ...
        return actions_list, goals_list, num_updates
    ...
    def wrap_horizon(self):
        ...
        for team, policy in zip(self.teams_list,self.policies_list):
            ...
            with torch.no_grad():
                ...
                next_high_value = torch.zeros((num_processes, num_agents), device=self.device)
    ...
    def eval_act(self, obs, env_states, goals):
        ...
        for team,policy,obs in zip(self.teams_list,self.policies_list,all_obs):
            ...
            # 占位符
            belief_map = torch.randn((1, 100, 100), device=self.device)  # [1, H, W]
            entropy_map = torch.randn((1, 100, 100), device=self.device)  # [1, H, W]
            ...
            # 占位符
            vec_inp = torch.randn((1, num_agents, 5), device=self.device)
            detected_map = torch.randn((100, 100), device=self.device)
            # [1, num_agents, 5]
            ...
            if goal_done_mask.any():
                ...
                # ⭐ 优化：直接在这里生成随机 vec_inp_agents 和 map_inps，作为占位符
                vec_inp_agents = torch.randn((len(proc_indices), 5), device=self.device)  # [N, 5]
                map_inps = torch.randn((len(proc_indices), 4, 100, 100), device=self.device)  # [N, 4, H, W]
                batch_goals = policy.get_random_high_level_goal(vec_inp_agents, map_inps)  # 需要实现batch版本
