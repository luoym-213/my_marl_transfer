...
def evaluate(args, seed, policies_list, ob_rms=None, render=False, env=None, master=None, render_attn=True):
    ...
    for t in range(num_eval_episodes):
        ...
        while not np.all(done):
            ...
            with torch.no_grad():
                ...
                #                                                                       landmark_data, 
                #                                                                       landmark_mask)
                actions, landmark_data, landmark_mask, entropy_map = master.eval_act(obs, env_states, mask, landmark_data, landmark_mask)

                
            episode_steps += 1
            #step_data = {'agents_actions': actions, 'agents_goals': goals, 'agents_tasks': tasks, 'graph_data': graph_data} 
            step_data = {'agents_actions': actions} 
    ...
    if successful_episodes_count > 0:
        ...
        successful_length_std = float(np.std(successful_episode_lengths))
    ...
    return (all_episode_rewards, per_step_rewards, all_high_episode_rewards, per_high_step_rewards, 
            final_min_dists, num_success, episode_length, successful_average_length, successful_length_std, successful_episodes_count,
...
if __name__ == '__main__':
    ...
    print(f"  Successful Episodes Avg Length: {successful_average_length:.2f} ± {successful_length_std:.2f} ({successful_episodes_count}/{args.num_eval_episodes})")
    print(f"\n【覆盖时间】")
