...
class Neo(object):
  ...
  def __init__(self, args, policy, obs_shape, action_space):
    ...
    self.rollouts = RolloutStorage(args.num_steps, args.num_processes, self.obs_shape, self.action_space, args.num_agents)
    self.args = args
  ...
  def initial_hidden_states(self, step):
    self.rollouts.recurrent_hidden_states[step].zero_()
